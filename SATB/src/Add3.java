...
//			System.out.println("fast.size():"+fast.size());//
			...
			for (int i = 1; i < fast.size(); i += 4) {
                                        //外层数组 指针一次前进4个
				for (int ii = 9; ii < cast.size(); ii += 12) {	//内层数组 指针一次前进12个
...
//							 + " ");
							...
							if (r) {
								...
								for (int zhuani = 0; zhuani < 16; zhuani++) {
...
//										
										+"  音名："+
										SC(cant[ii-8]) + " " + SC(cant[ii -7]) + " "
										+ SC(cant[ii -6]) + " " + SC(cant[ii -5])+"链接 " +
										SC(cant[ii-4]) + " " + SC(cant[ii -3]) + " "
										+ SC(cant[ii -2]) + " " + SC(cant[ii -1])+"链接 " +
