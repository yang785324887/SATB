...
/////////////////////////////根据一个已知的satb，构造下一个已知旋律的satb。
public class Add0 {

	// static int cfa = 29;//一个已知的satb的s
	// static int cfb = 26;//一个已知的satb的a
	// static int cfc = 24;//一个已知的satb的t
	// static int cfd = 15;//一个已知的satb的b

	static int cfa = 0;// 一个已知的satb的s
	static int cfb = 0;// 一个已知的satb的a
	static int cfc = 0;// 一个已知的satb的t
	static int cfd = 0;// 一个已知的satb的b
	...
	public static void A(String ct, String ft) { // 对比和声 处理关系
		...
		if (ckinput.ischord = true) {
			...
			// cant[ia] = Integer.parseInt(cast.get(ia));// 所有音所有声部排列的情况
			// // 转成int数组
			...
			for (int ia = 0; ia < fast.size(); ia++) {
				fant[ia] = Integer.parseInt(fast.get(ia));// 所有音 所有声部排列的情况
															// 转成int数组
			}
			// fai.remove(0);
			...
			// for (int ii = 0; ii < 4; i++) { // 内层数组 指针一次前进四个
				int ii = 0;
			...
			System.out.println(" 输入的四个数无法构成四部和声请重新输入");
		}// ckinput
	}
